...
pub(super) fn format_operands(operands: &[String], upper: bool) -> String {
    operands
        .iter()
        .map(|op| format_operand(op, upper))
        .collect::<Vec<_>>()
        .join(", ")
}

fn format_operand(s: &str, upper: bool) -> String {
    let s = s.trim();
    let s_lower = s.to_lowercase();
    for prefix in SIZE_PREFIXES {
        if s_lower.starts_with(prefix) {
            let rest = s[prefix.len()..].trim_start();
            let prefix_cased = if upper {
                prefix.to_uppercase()
            } else {
                prefix.to_string()
            };
            return format!("{} {}", prefix_cased, format_operand_inner(rest, upper));
        }
    }
    format_operand_inner(s, upper)
}

fn format_operand_inner(s: &str, upper: bool) -> String {
    let s = s.trim();
    if s.starts_with('[') && s.ends_with(']') {
        return format!("[{}]", format_mem_expr(&s[1..s.len() - 1], upper));
    }
    let has_operator = s.chars().any(|c| matches!(c, '+' | '-' | '*' | '/'));
    if has_operator {
        format_mem_expr(s, upper)
    } else {
        format_token(s, upper)
    }
}
...
fn format_mem_expr(s: &str, upper: bool) -> String {
    ...
    while let Some((i, c)) = chars.next() {
        match c {
            ...
            _ if c.is_alphanumeric() || matches!(c, '_' | '$' | '.' | '@') => {
                ...
                result.push_str(&format_token(&s[start..end], upper));
            }
            _ => {
                result.push(c);
            }
...
fn format_token(s: &str, upper: bool) -> String {
    let lower = s.to_lowercase();
    if REGISTERS.contains(&lower.as_str()) || SIZE_PREFIXES.contains(&lower.as_str()) {
        if upper { lower.to_uppercase() } else { lower }
    } else {
        s.to_string()
    }
}
...
mod tests {
    ...
    fn token_register_lowercased() {
        assert_eq!(format_token("RAX", false), "rax");
    }
    ...
    fn token_non_register_symbol_preserved() {
        assert_eq!(format_token("MyLabel", false), "MyLabel");
    }

    #[test]
    fn token_size_prefix_lowercased() {
        assert_eq!(format_token("BYTE", false), "byte");
    }

    #[test]
    fn token_number_unchanged() {
        assert_eq!(format_token("42", false), "42");
    }

    #[test]
    fn token_hex_lowercase_unchanged() {
        assert_eq!(format_token("0xff", false), "0xff");
    }

    #[test]
    fn token_register_uppercased() {
        assert_eq!(format_token("rax", true), "RAX");
    }

    #[test]
    fn token_size_prefix_uppercased() {
        assert_eq!(format_token("byte", true), "BYTE");
    }

    #[test]
    fn token_non_register_symbol_preserved_upper() {
        assert_eq!(format_token("MyLabel", true), "MyLabel");
    }
    ...
    fn operands_empty_list() {
        assert_eq!(format_operands(&[], false), "");
    }

    #[test]
    fn operands_single_register() {
        assert_eq!(format_operands(&ops(&["rax"]), false), "rax");
    }

    #[test]
    fn operands_two_registers() {
        assert_eq!(format_operands(&ops(&["rax", "rbx"]), false), "rax, rbx");
    }

    #[test]
    fn operands_three_items() {
        assert_eq!(
            format_operands(&ops(&["rax", "rbx", "rcx"]), false),
            "rax, rbx, rcx"
        );
    }
    ...
    fn operands_non_register_symbol_preserved() {
        assert_eq!(format_operands(&ops(&["MyLabel"]), false), "MyLabel");
    }

    #[test]
    fn operands_registers_uppercased() {
        assert_eq!(format_operands(&ops(&["rax", "rbx"]), true), "RAX, RBX");
    }

    #[test]
    fn operands_non_register_symbol_preserved_upper() {
        assert_eq!(format_operands(&ops(&["MyLabel"]), true), "MyLabel");
    }
    ...
    fn operands_size_prefix_byte() {
        assert_eq!(format_operands(&ops(&["byte [rax]"]), false), "byte [rax]");
    }

    #[test]
    fn operands_size_prefix_qword() {
        assert_eq!(
            format_operands(&ops(&["qword [rbp - 8]"]), false),
            "qword [rbp - 8]"
        );
    }
    ...
    #[test]
    fn operands_size_prefix_uppercased() {
        assert_eq!(format_operands(&ops(&["byte [rax]"]), true), "BYTE [RAX]");
    }
    ...
    fn operands_memory_arithmetic_adds_spaces() {
        assert_eq!(format_operands(&ops(&["[rbp-8]"]), false), "[rbp - 8]");
    }

    #[test]
    fn operands_memory_multiplication() {
        assert_eq!(
            format_operands(&ops(&["[rax+rbx*4]"]), false),
            "[rax + rbx * 4]"
        );
    }
    ...
    fn operands_unary_minus() {
        assert_eq!(format_operands(&ops(&["-1"]), false), "-1");
    }

    #[test]
    fn operands_memory_ref_uppercased() {
        assert_eq!(format_operands(&ops(&["[rbp - 8]"]), true), "[RBP - 8]");
    }
    ...
    fn operands_rel_keyword_space_preserved() {
        assert_eq!(format_operands(&ops(&["[rel msg]"]), false), "[rel msg]");
    }

    #[test]
    fn operands_abs_keyword_space_preserved() {
        assert_eq!(
            format_operands(&ops(&["[abs label]"]), false),
            "[abs label]"
        );
    }

    #[test]
    fn operands_rel_keyword_uppercased() {
        assert_eq!(format_operands(&ops(&["[rel msg]"]), true), "[REL msg]");
    }
    ...
    fn operands_double_quoted_string() {
        assert_eq!(format_operands(&ops(&["\"Hello\""]), false), "\"Hello\"");
    }
